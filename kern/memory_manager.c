...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	int space = ROUNDUP(size , PAGE_SIZE);
	int n_of_page = space / PAGE_SIZE;

	uint32 vs_of_page = virtual_address;
	for(int i = 0; i < n_of_page; i++)
	{
		//remove existing environment page at the given virtual address from page file
		//uint32 x= virtual_address+(i*PAGE_SIZE);
		pf_remove_env_page(e, vs_of_page);

		vs_of_page+=PAGE_SIZE;
	}
	...
	//int noPages = ROUNDUP(size , PAGE_SIZE) / PAGE_SIZE;
	uint32 f = virtual_address+(n_of_page*PAGE_SIZE);
	uint32* ptr_p_frame = NULL;
	struct Frame_Info* f_info =NULL;
	struct WorkingSetElement* element = NULL;
	LIST_FOREACH(element , &(e->SecondList))
	{
		if((element->virtual_address >= virtual_address) && (element->virtual_address < f))
		{
			...
			get_page_table(e->env_page_directory, (void*)element->virtual_address, &ptr_page_table);
			pt_clear_page_table_entry(e , element->virtual_address);
					if(ptr_page_table !=NULL)
					{
						...
						if(countt == 1024)
						{
							...
							f_info = get_frame_info(e->env_page_directory , (void*)ptr_page_table , &ptr_p_frame);
							f_info->references--;
							if (f_info->references == 0)
							{
								free_frame(f_info);
							}
							pd_clear_page_dir_entry(e,(uint32)element->virtual_address);
						}
	...
	struct WorkingSetElement* element_act = NULL;

	LIST_FOREACH(element_act , &(e->ActiveList))
	{
		//cprintf("111");
		if((element_act->virtual_address >= virtual_address) && (element_act->virtual_address < f))
		{
			//cprintf("222");

			LIST_REMOVE(&(e->ActiveList) , element_act);
			unmap_frame(e->env_page_directory,(void*) element_act->virtual_address);
			pt_set_page_permissions(e, element_act->virtual_address, 0, PERM_WRITEABLE |PERM_PRESENT|PERM_USER);
			LIST_INSERT_TAIL(&(e->PageWorkingSetList) , element_act);
			element_act->empty = 1;

			if (LIST_SIZE(&(e->SecondList)) != 0)
				{
				struct WorkingSetElement* head_second = LIST_FIRST(&(e->SecondList));
				LIST_REMOVE(&(e->SecondList), head_second);
				LIST_INSERT_TAIL(&(e->ActiveList) , head_second);


				pt_set_page_permissions(e, head_second->virtual_address,PERM_WRITEABLE |PERM_PRESENT|PERM_USER , 0);



				}

			uint32 *ptr_page_table_act=NULL;
			get_page_table(e->env_page_directory, (void*)element_act->virtual_address, &ptr_page_table_act);
			pt_clear_page_table_entry(e , element_act->virtual_address);

			if(ptr_page_table_act != NULL)
			{

				int countt =0;
				for(int j=0; j<1024; j++)
				{
					if(ptr_page_table_act[j] == 0)
					{
						countt++;
					}
				}
				if(countt == 1024)
				{
					//unmap_frame(e->env_page_directory,ptr_page_table_act);

					f_info = get_frame_info(e->env_page_directory , (void*)ptr_page_table_act , &ptr_p_frame);
					f_info->references--;
					if (f_info->references == 0)
					{
						free_frame(f_info);
					...
					pd_clear_page_dir_entry(e,(uint32)element_act->virtual_address);
